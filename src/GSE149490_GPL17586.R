...
transfer_tab <- get_IDtransfer(from_type = 'affy_primeview',to_type = 'external_gene_name')
transfer_tab <- dplyr::rename(transfer_tab,ID = affy_primeview)
transfer_tab <- transfer_tab[!duplicated(transfer_tab$ID),] #删除一个探针对应多个基因名的探针
transfer_tab_biotype <- read.csv(file = "/cluster/home/chenhy/project/neuron_drivers_landscape/result/transfer_tab.csv") %>% .[which(.$gene_biotype=="protein_coding"),]
transfer_tab <- transfer_tab[which(transfer_tab$external_gene_name %in% transfer_tab_biotype$external_gene_name),]
...
# Remove batch effect
phe_info <- pData(network.par$net.eset)
#modDx=model.matrix(~phe_info$group +phe_info$age +phe_info$material)#model to pass to num.sv to determine the number of PCs to include in the model-- when explicitly modeling these variables
#k = num.sv(mat, modDx) #passing our data and model to the num.sv function of the bioconductor package sva to determine the number of PCs sig contributing to overall variance
#PCA<-prcomp(t(mat)) # getting the PCs
design <- model.matrix(~phe_info$group)
#batch <- model.matrix(~)
batch <- phe_info$`hybridization batch`
batch2 <- phe_info$material
covariates <- model.matrix(~phe_info$age +phe_info$`postmorten interval (h)`)#+PCA$x[,1:k]
limma.edata <- removeBatchEffect(network.par$net.eset,
                                 design = design,
                                 batch = batch,
                                 batch2 = batch2,
                                 covariates = covariates)
# Update eset
net_eset <- generate.eset(exp_mat=limma.edata, phenotype_info=phe_info,
                          feature_info=fData(network.par$net.eset)[rownames(limma.edata),], 
